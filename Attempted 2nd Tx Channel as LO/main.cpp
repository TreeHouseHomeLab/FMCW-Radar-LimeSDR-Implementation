...
int Stream(){
    ...
    for (int i = 0; i < chCount; ++i){
            ...
            lo_streams.channel = 0; //channel number
    ...
    bufferlo = new float[(chirp_size +deadsamp) * 2];
    ...
    for (int i = 0; i < chCount; ++i)
        ...
        LMS_DestroyStream(device, &rx_streams[i]); //stream is deallocated and can no longer be used
         if(i<1){
            ...
            LMS_DestroyStream(device, &lo_streams);}
